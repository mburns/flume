...
node[:flume][:multi_agent][:count].times.map do |index|
  ...
  #
  announce(:flume, :agent,       logs: { node: { glob: File.join(log_dir, 'flume-flume-node*.log'), logrotate: false, archive: false } },
                                 ports: { status: { port: 35_862 + index, protocol: 'http', dashboard: true }  },
